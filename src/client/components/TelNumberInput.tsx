...
export const TelNumberInput: React.FC<TelNumberInputProps> = ({ telNumber, isTelConfirmed, setTelConfirmed }) => {
  ...
  const requestTelCode = async (telNumber: string) => {
    let response = await fetch("/SignInStep2/RequestTelCode", {
      method: "POST",
      headers: {
        "Content-Type": "application/json;charset=utf-8",
      },
      body: JSON.stringify({ telNumber: telNumber }),
    });
  ...
  const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    setIsCodeSended(!isCodeSended);
    e.preventDefault();
    // userDataStore.setUserTel(telNumber.value);
    requestTelCode(telNumber.value);
  };

  const checkCode = async (code: string) => {
    let response = await fetch("/SignInStep2/CheckTelCode", {
      method: "POST",
      headers: {
        "Content-Type": "application/json;charset=utf-8",
      },
